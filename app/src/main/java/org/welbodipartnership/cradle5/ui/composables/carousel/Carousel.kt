 ...
 * From https://github.com/sahruday/Carousel/blob/master/carousel/src/main/java/com/sahu/carousel/Carousel.kt
 * with modifications based on https://stackoverflow.com/a/68056586 to make it hide when not scrolling
 *
 *
...
fun Carousel(
  ...
  var startHiding by remember { mutableStateOf(false) }
  // delay hiding; show bar if updates
  LaunchedEffect(state.isScrollInProgress, state.layoutInfo.totalItemsCount) {
    if (state.isScrollInProgress) {
      if (startHiding) startHiding = false
    } else {
      startHiding = false
      delay(600L)
      startHiding = true
  ...
  val targetAlpha = if (state.isScrollInProgress || !startHiding) 1f else 0f
